...
现在，2022软挑也落下帷幕，作为两届老玩家，也是学生时代的最后一个比赛，有遗憾也有进步，这里总结一下我队方案：
...
组队即将截止时余哥找上了我，余哥也是去年的参赛选手之一，于是我俩一拍即合，今年决定再好好整一整，余哥这个大腿抱的好才有精彩的比赛体验。合并代码后又做了一些新思路的调整，但是没上什么分。不过，按去年经验复赛题目会变化比较大，因此也不比过分纠结初赛的结果，能进个复赛就行。
...
2. 预分配选择免费的5%时刻中，初赛取吸收流量最大的时刻。为了优先考虑**度小但需求大**的客户，取 **(需求大小/客户的度)之和** 最大的的时刻（这个在复赛阶段对分数有一定影响）。
...
题意中，每个边缘节点的最大带宽为100 0000，也就是最多经过4次5%衰减后缓存值的变化会<1。因此我们约束只取到t+4。同理，为了加快运算，更新缓存也可只更新未来5个时刻。然而，一种corner case是**缓存一直会+1直到最后一个时刻**，为了解决这种极端情况，只能更新到最后一个时刻，显然代价过大得不偿失。我们稍后再处理这种情况。
